...
local filename "IV Urban T-dum Tenure 1979 3 years"
...
*1)
ivregress 2sls log_monthlyearning tenure tenuresqr male (higheredu secgenhighedu secgenedu = tdum2-tdum265) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
...
ivregress 2sls log_monthlyearning tenure tenuresqr male provincedum2-provincedum15 (higheredu secgenhighedu secgenedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr (higheredu secgenhighedu secgenedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr provincedum2-provincedum15 (higheredu secgenhighedu secgenedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr (higheredu secgenhighedu secgenedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr provincedum2-provincedum15 (higheredu secgenhighedu secgenedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
*2)
ivregress 2sls log_monthlyearning tenure tenuresqr male (higheredu secgenhighedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr male provincedum2-provincedum15 (higheredu secgenhighedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr (higheredu secgenhighedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr provincedum2-provincedum15 (higheredu secgenhighedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr (higheredu secgenhighedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr provincedum2-provincedum15 (higheredu secgenhighedu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
*3)
ivregress 2sls log_monthlyearning tenure tenuresqr male (higheredu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr male provincedum2-provincedum15 (higheredu  = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr (higheredu  = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr provincedum2-provincedum15 (higheredu  = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr (higheredu  = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning tenure tenuresqr provincedum2-provincedum15 (higheredu = tdum2-tdum265 ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
