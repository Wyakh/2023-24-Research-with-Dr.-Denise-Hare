...
local filename "IV Rural ATF&Birth Exp 1979 3years"
...
*1)
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 male (higheredu secgenhighedu secgenedu = t d td) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & educat != ., first
estat endog
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 male provincedum2-provincedum15 (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 provincedum2-provincedum15 (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 provincedum2-provincedum15 (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
*2)
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 male (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 male provincedum2-provincedum15 (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 provincedum2-provincedum15 (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 provincedum2-provincedum15 (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
*3)
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 male (higheredu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 male provincedum2-provincedum15 (higheredu  = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 (higheredu  = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 provincedum2-provincedum15 (higheredu  = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 (higheredu  = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr birthmonthdum2-birthmonthdum12 provincedum2-provincedum15 (higheredu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 200, 16667) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
