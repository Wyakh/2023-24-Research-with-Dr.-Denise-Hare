...
local filename "IV Urban ATF Exp 1982 3years"
...
*1)
ivregress 2sls log_monthlyearning exp expsqr male (higheredu secgenhighedu secgenedu = t d td) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
...
ivregress 2sls log_monthlyearning exp expsqr male provincedum2-provincedum15 (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr provincedum2-provincedum15 (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr provincedum2-provincedum15 (higheredu secgenhighedu secgenedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
*2)
ivregress 2sls log_monthlyearning exp expsqr male (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr male provincedum2-provincedum15 (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr provincedum2-provincedum15 (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr provincedum2-provincedum15 (higheredu secgenhighedu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
*3)
ivregress 2sls log_monthlyearning exp expsqr male (higheredu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr male provincedum2-provincedum15 (higheredu  = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr (higheredu  = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr provincedum2-provincedum15 (higheredu  = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 1 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr (higheredu  = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
...
ivregress 2sls log_monthlyearning exp expsqr provincedum2-provincedum15 (higheredu = t d td ) if inrange(t, -`months', `months'-1) & inrange(monthlyearning, 250, 25000) & male == 0 & educat != ., first
estat endog
local durbin_p = r(p_durbin)
local wu_p = r(p_wu)
local endog_df = r(df)
